...
int init_all_micro_bench(short int type_id)
{
	...
	ret = init_micro_bench(&(vector_mb[0]), "micro-benchmark_ch0" ,
				boot_mem_microBenchmark , temp_size , type_id , 0);
	if(ret < 0)
		return -ENOMEM;	
	
	ret = init_micro_bench(&(vector_mb[1]), "micro-benchmark_ch1" ,
				&(boot_mem_microBenchmark[temp_size]) , temp_size, type_id , 1);
	if(ret < 0)
		goto err_mb1;
	
	ret = init_micro_bench(&(vector_mb[2]), "micro-benchmark_ch2" ,
				&(boot_mem_microBenchmark[temp_size * 2]) , temp_size , type_id , 2);
	if(ret < 0)
		goto err_mb2;	
		
	ret = init_micro_bench(&(vector_mb[3]), "micro-benchmark_ch3" ,
				&(boot_mem_microBenchmark[temp_size * 3]) , temp_size , type_id , 3);
	if(ret < 0)
		goto err_mb3;
...
int micro_bench_print_info(void)
{
	...
	switch (type_measure) 
	{
		...
		{
			if(start_policy_prio2(_NANOSEC_HIGH , _NANOSEC_LOW) < MB_MINOR)
			{
				printk(KERN_WARNING "%s: Error start priority policy\n", DRIVER_NAME);
				return -EIO;
			}

			printk(KERN_INFO "%s: Micro-Benchmark on DMA (priority policy) \n", DRIVER_NAME);
		}
		break;
