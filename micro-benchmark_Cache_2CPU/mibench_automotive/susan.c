...
#ifdef __GNUC__
    #define NOWARN_IF_UNUSED __attribute__((unused))
#else
    #define NOWARN_IF_UNUSED
#endif
/* }}} */
/* {{{ usage() */
  ...
  FILE *fd;
{
  int c, i;
  char dummy[10000];
  char *unused NOWARN_IF_UNUSED;

  c = getc(fd);
  while (1) /* find next integer */
  {
    if (c=='#')    /* if we're at a comment, read to end of line */
    unused = fgets(dummy,9000,fd);
    if (c==EOF)
...
uchar  * setup_brightness_lut(bp,thresh,form)
  uchar **bp;
  int   thresh, form;
{
int   k;
...
/* }}} */
  ...
  tmp_image = (uchar *) malloc( (x_size+mask_size*2) * (y_size+mask_size*2) );   //ok
...
void susan_thin(r,mid,x_size,y_size)
  ...
  int   *r, x_size, y_size;
{
int   l[9], centre, nlinks  NOWARN_IF_UNUSED , npieces  NOWARN_IF_UNUSED,
      b01, b12, b21, b10,
      p1, p2, p3, p4,
      b00, b02, b20, b22,
      m, n, a, b, x, y, i, j;
...
uchar c,*p,*cp;
  ...
  cgx=(int *)malloc(x_size*y_size*sizeof(int)); //ok
  cgy=(int *)malloc(x_size*y_size*sizeof(int));  //ok
...
void start_susan(int argc, char argv[N_ARG][SIZE_ARG] , struct mi_result *result)
{
/* {{{ vars */

FILE   *ofp  NOWARN_IF_UNUSED;
char   filename [80]  NOWARN_IF_UNUSED,
       *tcp;
uchar  *in, *bp, *mid, *tmp_bp;
float  dt=4.0;
int    *r,
       argindex=3,
       bt=20,
...
/* }}} */
  ...
  switch (mode)
  {
    case 0:
      /* {{{ smoothing */
			tmp_bp = setup_brightness_lut(&bp,bt,2);
...
/* }}} */
    case 1:
      ...
      tmp_bp = setup_brightness_lut(&bp,bt,6);
      ...
      {
        mid = (uchar *)malloc(x_size*y_size); //ok
...
/* }}} */
    case 2:
      ...
      r   = (int *) malloc(x_size * y_size * sizeof(int));  //ok
      tmp_bp = setup_brightness_lut(&bp,bt,6);
